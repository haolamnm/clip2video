...
def extract_frames(video_name, out_folder, fps=5):
    ...
    cmd = "ffmpeg -v 0 -i %s -r %d -q 0 %s/%s.jpg" % (
        video_name,
        fps,
        out_folder,
        "%08d",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, default="input", help="input directory of videos"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="output",
        help="output directory of sampled frames",
    )
    ...
    lines = [
        (
            os.path.join(args.input_path, mp4),
            os.path.join(args.output_path, mp4.split(".")[0]),
        )
        for mp4 in mp4_file
    ]
