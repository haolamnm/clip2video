...
class RawFrameExtractor:
    ...
    def video_to_tensor(self, video_file, max_frame, preprocess, sample_fp=0):
        ...
        current_sample_indx = np.linspace(
            0, len(video_name) - 1, num=current_frame, dtype=int
        )
        ...
        if max_frame >= current_sample_indx.shape[0]:
            ...
            frame_index = np.linspace(
                0, current_sample_indx.shape[0] - 1, num=max_frame, dtype=int
            )
            ...
            if self.train:
                ...
                if step_len > 2:
                    random_index = np.random.randint(
                        -1 * step_len, step_len, (frame_index.shape[0] - 2)
                    )
        ...
        for index in frame_index:
            image_path = os.path.join(
                video_file, video_name[current_sample_indx[int(index)]]
            )
    ...
    def get_video_data(self, video_path, max_frame):
        ...
        image_input = self.video_to_tensor(
            video_path, max_frame, self.transform, sample_fp=self.framerate
        )
